class chatRoom{
    constructor(roomNameInput, roomDescriptionInput){
        ...
        //Creating the room
        Chatrooms[this.roomNameInput] = {"description" : roomDescriptionInput, "messages" : [], "users": [], "object" : this};
        this.displayRoom();
    ...
    selectRoom(roomDiv){
        ...
        this.roomDescriptionDiv.innerText = Chatrooms[this.roomNameInput]['description']
        ...
        userUL.innerHTML = '<img src="assets/plus-symbol-button.png" alt="add user" class="add-user-button">';
    ...
    displayRoom(){
        ...
        roomName.addEventListener('click', function(){
            Chatrooms[roomName.innerText]["object"].selectRoom(roomName);
        })

        deleteRoomButton.addEventListener('click', function(){
            Chatrooms[roomName.innerText]["object"].deleteRoom(deleteRoomButton);
        })    
    }
    ...
    deleteRoom(deleteButton){
        ...
        if ([...remove.classList].includes("selected")){
            if(remove.nextElementSibling !== null){
                newSelected = Chatrooms[remove.nextElementSibling.innerText]["object"]                
                newSelected.selectRoom(remove.nextElementSibling.firstChild);
                remove.remove();
            } else if (remove.previousElementSibling !== null){
                newSelected = Chatrooms[remove.previousElementSibling.innerText]["object"]                
                newSelected.selectRoom(remove.previousElementSibling.firstChild);
                remove.remove();
            } else{
                this.roomDescriptionDiv.innerText = "";
                const chatTable = document.querySelector('table');        
    ...
    showMessages(){
        let messagesList = Chatrooms[this.roomNameInput]["messages"];
    ...
    sendMessage(){
        ...
        if (messageText != ""){
            ...
            const selectedRoom = document.querySelector('.selected');
            Chatrooms[selectedRoom.innerText]["messages"].push([name.innerText, messageText, date, currentTime]);
        
            this.createMessageRow([name.innerText, messageText, date, currentTime]);
        }
    }
    ...
    showUsers(){
        let userList = Chatrooms[this.roomNameInput]["users"];
